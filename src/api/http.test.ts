...
describe('HTTPTransport', () => {
  ...
  test('get() calls fetch with GET and returns parsed JSON', async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(expect.stringContaining('/test'), expect.objectContaining({ method: 'GET', credentials: 'include' }))
    expect(res.ok).toBe(1)
  })

  test('post() sends JSON by default', async () => {
    ;(globalThis.fetch as jest.Mock).mockResolvedValue({ ok: true, status: 200, json: async () => ({ created: true }) })
    const http = new HTTPTransport()
    const body = { a: 1 }
  ...
  test('post() keeps FormData without setting Content-Type', async () => {
    ;(globalThis.fetch as jest.Mock).mockResolvedValue({ ok: true, status: 200, json: async () => ({ ok: 1 }) })
    const http = new HTTPTransport()
  ...
  test('throws parsed error body on non-ok response', async () => {
    ;(globalThis.fetch as jest.Mock).mockResolvedValue({ ok: false, status: 400, json: async () => ({ reason: 'bad' }) })
    const http = new HTTPTransport()
